...
FitFindr takes a user's natural-language request — style preferences, size, and budget — and runs it through three tools in sequence: `search_listings` is triggered first to find matching secondhand items, then `suggest_outfit` is triggered once a listing is selected to pair it with pieces the user already owns, and finally `create_fit_card` is triggered to format everything into a shareable outfit summary. If `search_listings` returns nothing, the agent asks the user to relax a constraint and retries rather than continuing; if `suggest_outfit` receives an empty wardrobe, it skips pairings and passes the new item alone to `create_fit_card` so the user still gets a useful output; if `create_fit_card` encounters missing listing fields, it substitutes "N/A" and renders the card anyway with a warning.
