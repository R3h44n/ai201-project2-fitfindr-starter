...
**Step 1:**
The agent parses the query and extracts: `style_keywords=["vintage", "graphic tee"]`, `size=None` (not specified), `max_price=30.0`. It calls `search_listings(style_keywords=["vintage", "graphic tee"], size=None, max_price=30.0)`.

The tool scans all 40 listings and returns two matches sorted by price:
- `lst_006` — "Graphic Tee — 2003 Tour Bootleg Style", $24, size L, tags: `["graphic tee", "vintage", "grunge", "streetwear", "band tee"]`
- `lst_033` — "Vintage Band Tee — Faded Grey", $19, size L, tags: `["vintage", "grunge", "band tee", "graphic tee", "streetwear"]`

The agent picks the cheapest option: `lst_033` at $19. It sets `state["selected_item"] = lst_033` and `state["last_search_results"] = [lst_033, lst_006]`.

**Step 2:**
The agent calls `suggest_outfit(new_item=lst_033, wardrobe=example_wardrobe)`.

`lst_033` is a `tops` item with colors `["grey", "charcoal"]` and style tags `["vintage", "grunge", "band tee"]`. The tool looks for wardrobe items in the other categories:
- **Bottoms**: `w_001` (baggy dark wash jeans) — dark indigo pairs cleanly with grey; both have `streetwear` tags. Selected.
- **Shoes**: `w_008` (black combat boots) — grunge tag matches; black anchors the charcoal tones. Selected.
- **Accessories**: `w_010` (black crossbody bag) — minimal, doesn't compete with the graphic. Selected.

Returns: `{"outfit": [w_001, w_008, w_010], "rationale": "The faded grey tee anchors a classic grunge streetwear look. Dark wash baggy jeans echo the worn-in vibe without competing with the graphic. Black combat boots and a simple crossbody keep the focus on the tee while grounding the palette."}`.

The agent updates `state["last_outfit"]` and `state["last_rationale"]`.

**Step 3:**
The agent calls `create_fit_card(new_item=lst_033, outfit=[w_001, w_008, w_010], rationale="...")`.

The tool formats and returns the fit card string (see template above).
...
╠══════════════════════════════════════════╣
║  WHY IT WORKS                           ║
║  The faded grey tee anchors a classic   ║
║  grunge streetwear look. Dark wash      ║
║  baggy jeans echo the worn-in vibe      ║
║  without competing with the graphic.    ║
║  Black combat boots and a simple        ║
║  crossbody keep the focus on the tee   ║
║  while grounding the palette.           ║
╚══════════════════════════════════════════╝

Also found: "Graphic Tee — 2003 Tour Bootleg Style" ($24, depop) — want me to style that one instead?
```
